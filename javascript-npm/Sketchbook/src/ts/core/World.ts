...
import { CarClip } from '../../../02-gameSystem/src/clips/car.js';
import { Masna_01 } from '../../../02-gameSystem/src/clips/masna_01.js';
...
export class World
{
    ...
    public update(timeStep: number): void
    {
        // jade
        if(this.clip){
            this.clip.globals.time += timeStep;
            this.clip.update();
        }
    ...
    public loadScene(gltf: any): void
    {
                ...
                if (child.userData.hasOwnProperty('data'))
                {
                    ...
                    if (child.userData.data === 'spawn')
                    {
                        ...
                        else if (child.userData.type === 'airplane')
                        {
                            ...
                            {
                                // jade
                                //car child position {x: 1.370016098022461, y: 14.803786277770996, z: 9.11441421508789}
                                child.position.set(11.37, 14.80, 9.11);
        ...
        // joseangel
        const rendererDeMentira = new THREE.WebGLRenderer();
        const clip = new Masna_01(rendererDeMentira,{globals: {time:0,deltaTime:0}});
        this.clip = clip;
        console.log(clip);
        const self = this;
        const meshes = [];
