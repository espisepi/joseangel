...
function main() {
	...
	void mainImage( out vec4 fragColor, in vec2 fragCoord )
	{
		...
		//uv.x *= iResolution.x / iResolution.x;

		//vec4 texture = texture2D(iChannel0, floor(uv * float(TILES)) / float(TILES));
	...
	vec3 first(vec3 position) {
		vec3 res = position;
		
		float limite = sin(iTime*2.0);
		float movz = step(limite,res.y);
		res.z += movz;
		return res;
	...
	void main() {
		...
		vec3 newPosition = first(position);
	...
	const texture = loader.load('textures/foto.jpg');
